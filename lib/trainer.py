...
from .utils.cliff_utils import convert_to_angle_axis
...
class Trainer():
	...
	def visualise(self):
		...
		img_bgr = cv2.imread(os.path.join(
			self.cfg.DATA.VAL_PATH, "images", random_file))
	...
	def validate(self):
		...
		for i, (batch) in enumerate(self.val_loader):
			...
			target_landmarks = batch["target_landmarks"].to(
				self.device).float()
			...
			projected_keypoints_2d = projected_keypoints_2d[:,
															all_smplx_indices, :] / img_shape.unsqueeze(1)
			...
			keypoint_loss = compute_keypoint_loss(
				projected_keypoints_2d, target_landmarks)
	...
	def fit(self):
		...
		for epoch in range(start_epoch, self.cfg.TRAIN.EPOCHS):
			for idx, batch in enumerate(self.train_loader):
				...
				keypoint_loss = compute_keypoint_loss(
					projected_keypoints_2d, target_landmarks)
				beta_loss = compute_beta_loss(
					pred_betas, beta_params, is_params)
				pose_loss = compute_pose_loss(
					pred_rotmat, pose_params, is_params)
